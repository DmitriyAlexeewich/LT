...
public class DropEquip : MonoBehaviour {
	public HotContainer HotContainerScr;
	public Inv InvScr;
	Decode Decoder = new Decode();
	public void OnClick(){
		if (HotContainerScr.Engaged == true) {
			GameObject TObject = Instantiate (Resources.Load<GameObject>("Prefabs/"+HotContainerScr.ItemScr.ID.ToString()));
			TObject.transform.position = HotContainerScr.InvScr.DropPoint.transform.position;
			TObject.GetComponent<Item> ().Array = HotContainerScr.ItemScr.Array;
